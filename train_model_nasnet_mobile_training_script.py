...
MODEL_DIR = "model_nasnet"
MODEL_PATH = os.path.join(MODEL_DIR, "nasnet_model.keras")
CLASS_NAMES_PATH = os.path.join(MODEL_DIR, "class_names.json")
HISTORY_PATH = os.path.join(MODEL_DIR, "training_history.json")

# CRITICAL: Use 224x224 for NASNetMobile
IMAGE_SIZE = (224, 224)
BATCH_SIZE = 32
EPOCHS = 100
...
def load_and_preprocess_image(img_path, label):
    """Load image with proper NASNetMobile preprocessing"""
    ...
    # NASNet expects values in [-1, 1] range
    img = tf.cast(img, tf.float32)
    img = applications.nasnet.preprocess_input(img)
...
def augment_image(img, label):
    ...
    # Clip values to valid range for NASNet
...
print(f"✅ Loaded NASNetMobile (ImageNet weights)")
...
model = keras.Model(inputs, outputs, name='nasnet_classifier')

print(f"\n📊 Model architecture:")
print(f"   Input: {IMAGE_SIZE[0]}x{IMAGE_SIZE[1]}x3")
print(f"   Base: NASNetMobile (frozen)")
...
# NASNetMobile has many layers, freeze bottom 70%
